...
Send only one heatmap. If you are unsure, check the help document on this function by typing ?heatmap.2

```{r}
png("problem1_AnnaSemik.png",height=700,width=700)
heatmap.2(edata,
          main = "Bottomly et al. Clustered", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
...
```{r}
png("problem2_AnnaSemik.png",height=300,width=400)
...
```{r}
PCloadings <- data.table(svd.out$u,pData(bottomly.eset))
png("problem3_AnnaSemik.png",height=300,width=500)
ggplot(PCloadings) + geom_point(aes(x=V1, y=V2, col=as.factor(strain)))
dev.off()
```
...
```{r}
png("problem4_AnnaSemik.png",height=500,width=600)
top5 <- PCloadings[,c(1:5, 24)]
top5lsv <- melt(t5)
ggplot(top5lsv) + geom_violin(aes(x=as.factor(variable), y=value),draw_quantiles = c(0.25, 0.5, 0.75))
dev.off()
```
